...
class BaseDetailParser(ABC):
    ...
    def parse(self, soup: BeautifulSoup | None, item: dict, source_name: str = "") -> DetailData:
        ...
        rough_title = bool(item.get("_rough_title")) or data.slug.isdigit()
        ...
        # A slug-derived or numeric listing title is replaced by the real heading once we have it.
        if rough_title and data.title == listing_title and data.post_name and len(data.post_name) > 10:
            data.title = data.post_name
        if data.title and not data.title.isdigit():
            data.slug = _slugify(data.title)
