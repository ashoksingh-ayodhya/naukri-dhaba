...
export default async function AdmitCardCategoryPage({ params }: Props) {
  ...
  return (
    <>
      <ListingSchema title={`${cat?.fullName || category} Admit Cards ${YEAR}`} path={`/admit-cards/${category}/`} posts={posts} crumbs={[{ label: "Home", href: "/" }, { label: "Admit Cards", href: "/admit-cards/" }, { label: cat.label }]} />
