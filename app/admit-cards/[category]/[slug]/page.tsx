...
import { buildMetadata, buildBreadcrumbJsonLd, buildAdmitJsonLd } from "@/lib/seo";
...
export default async function AdmitCardDetailPage({ params }: Props) {
  ...
  const breadcrumbs = [
    ...
    { label: cat?.label || category, href: `/admit-cards/${category}/` },
    { label: fm.title },
  ];

  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(buildAdmitJsonLd(fm, pageUrl)) }} />
